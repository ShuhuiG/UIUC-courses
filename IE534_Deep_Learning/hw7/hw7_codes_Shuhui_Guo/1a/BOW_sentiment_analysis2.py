...
from BOW_model2 import BOW_model2
...
model = BOW_model2(vocab_size,500)
model.cuda()
...
torch.save(model,'BOW21.model')
data = [train_loss,train_accu,test_accu]
data = np.asarray(data)
np.save('data21.npy',data)
save = pd.DataFrame({"Train_loss" : np.array(train_loss), "Train_accu" : np.array(train_accu), "Test_accu" : np.array(test_accu)})
save.to_csv("save21.csv", index=False)

...
parser = argparse.ArgumentParser(description='GAN CIFAR-10')
parser.add_argument('--resume', '-r', default = "./checkpoint.pth.tar", help='resume')
parser.add_argument('--epochs_total', '-n', default=300, type=int, help='all epoch')
...
class generator(nn.Module):
    def __init__(self):
        ...
        # conv1
        self.conv1 = nn.ConvTranspose2d(in_channels = 128, out_channels = 128,
        ...
        self.relu4 = nn.ReLU()
        # conv5
        self.conv5 = nn.ConvTranspose2d(in_channels = 128, out_channels = 128,
                                        kernel_size=4, stride=2, padding=1, bias=False)
        ...
        self.relu6 = nn.ReLU()
        # conv7
        self.conv7 = nn.ConvTranspose2d(in_channels = 128, out_channels = 128,
                                        kernel_size=4, stride=2, padding=1, bias=False)
        self.bn7 = nn.BatchNorm2d(128)
        self.relu7 = nn.ReLU()
        # conv8
        self.conv8 = nn.Conv2d(in_channels = 128, out_channels = 3,
                                kernel_size=3, stride=1, padding=1, bias=False)
    def forward(self, x):
        x = self.fc1(x)
        x = x.view(-1, 128, 4, 4)
...
if args.resume:
    ...
    if os.path.isfile(args.resume):
        ...
        epoch_initial = checkpoint['epoch'] + 1
...
# Train the model
for epoch in range(epoch_initial, args.epochs_total):
    aG.train()
    aD.train()
    ...
    for batch_idx, (X_train_batch, Y_train_batch) in enumerate(trainloader):
        ...
        noise = np.random.normal(0,1,(batch_size_train,n_z))
    ...
    with torch.no_grad():
        ...
        test_accu_epoch.append(accuracy_test)
...
save.to_csv("part2.csv", index=False)
